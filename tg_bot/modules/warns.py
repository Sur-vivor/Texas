...
from tg_bot import dispatcher, BAN_STICKER, WHITELIST_USERS, B_USERS
...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if user.id in B_USERS:
        if warner:
            message.reply_text("B cant be warned.")
        else:
            message.reply_text("B triggered an auto warn filter!\n I can't warn B but they should avoid abusing this.")
        return

    if user.id in WHITELIST_USERS:
        if warner:
            message.reply_text("Wolf tiers are warn immune.")
        else:
            message.reply_text("Wolf Tier triggered an auto warn filter!\n I can't warn C but they should avoid abusing this.")
        return
    ...
    if num_warns >= limit:
        ...
        message.bot.send_sticker(chat.id, BAN_STICKER)  # Texas's sticker
        keyboard = []
