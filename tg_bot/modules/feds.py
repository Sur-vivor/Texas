...
def new_fed(bot: Bot, update: Update):
    ...
    if user.id == int(OWNER_ID):
        ...
        if not fed:
            update.effective_message.reply_text("Can't federate! Please contact @texassupport if the problem persists.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not fed:
            message.reply_text("Failed to join federation! Please contact @texassupport should this problem persists!")
            return

        get_fedlog = sql.get_fed_log(args[0])
        if get_fedlog:
            bot.send_message(get_fedlog, "Chat *{}* has joined the federation *{}*".format(chat.title, getfed['fname']), parse_mode=ParseMode.MARKDOWN)
...
def leave_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if not getuser in 'creator' or not user.id in SUDO_USERS:
        update.effective_message.reply_text("Only group creators can use this command!")
        return

    if not sql.chat_leave_fed(chat.id):
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if not is_user_fed_owner(fed_id, user.id) or not user.id in DEV_USERS:
        update.effective_message.reply_text("Only federation owners can do this!")
        return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if not x:
        message.reply_text("Failed to ban from the federation! If this problem continues, contact @texassupport.")
        return

    fed_chats = sql.all_fed_chats(fed_id)
    # Send message to owner if fednotif is enabled
    log_message = "<b>FedBan reason updated</b>" \
                  "\n<b>Federation:</b> {}" \
                  "\n<b>Federation Admin:</b> {}" \
                  "\n<b>User:</b> {}" \
                  "\n<b>User ID:</b> <code>{}</code>" \
                  "\n<b>Reason:</b> {}".format(fed_name, mention_html(user.id, user.first_name),
                                               user_target, fban_user_id, reason)

    if getfednotif:
        bot.send_message(info['owner'], log_message, parse_mode=ParseMode.HTML)

    get_fedlog = sql.get_fed_log(fed_id)
    if get_fedlog:
        if int(get_fedlog) != int(chat.id):
            bot.send_message(get_fedlog, log_message, parse_mode=ParseMode.HTML)
    ...
    for fedschat in fed_chats:
        ...
        try:
            ...
            if excp.message in FBAN_ERRORS:
                try:
                    bot.getChat(fedschat)
                except Unauthorized:
                    sql.chat_leave_fed(fedschat)
                    LOGGER.info("Chat {} has left fed {} because I was kicked".format(fedschat, info['fname']))
                    continue
                except BadRequest:
                    pass
...
def unfban(bot: Bot, update: Update, args: List[str]):
    ...
    if subscriber:
        for fedsid in subscriber:
            ...
            for fedschat in all_fedschat:
                try:
                    bot.unban_chat_member(fedschat, user_id)
                except BadRequest as excp:
                    if excp.message in FBAN_ERRORS:
                        try:
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if not x:
        update.effective_message.reply_text("Big F! There is an error while setting federation rules! If you wondered why please ask it in @texassupport !")
        return
